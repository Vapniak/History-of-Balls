      ...
      foreach (var type in assembly.GetTypes()) {
        ...
        foreach (MethodInfo method in methods) {
          ...
          foreach (var attribute in attributes) {
            if (attribute is not ConsoleCommandAttribute consoleCommandAttribute)
              continue;

            if (Commands.FirstOrDefault(x => x.Command == consoleCommandAttribute.Command) != null) {
              throw new Exception($" Duplicate console command: {consoleCommandAttribute.Command}");
            }
            ...
            GD.Print($" Loaded command {consoleCommandAttribute.Command}");
          }
        }
      }
    }
