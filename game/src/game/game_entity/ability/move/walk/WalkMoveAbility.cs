  ...
  public new partial class Instance : MoveAbility.Instance {
    ...
    public override bool CanActivateAbility(GameplayEventData? eventData) {
      if (eventData?.TargetData is MoveTargetData moveTargetData) {
        _path = FindPathTo(moveTargetData.Cell);
        return _path.Length > 0 && _path.Last() != OwnerEntity.Cell && base.CanActivateAbility(eventData);
      }
      return base.CanActivateAbility(eventData);
    }

    public override void ActivateAbility(GameplayEventData? eventData) {
      if (_path.Length > 0 && CommitCooldown()) {
